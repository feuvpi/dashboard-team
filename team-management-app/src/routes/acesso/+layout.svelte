<script lang="ts">
	...
	const style = {
		...
		container: `dark:bg-neutral-800 bg-neutral-100 dark:bg-background h-screen overflow-hidden relative lg:p-4 dark:bg-card dark:text-card-foreground`,
...
<div class={style.container}>
	<div class="flex items-start">
		...
		<div class={`${style.mainContainer} ${$sidebarOpen ? style.open : style.close}`}>
			...
			<header
				class="bg-white dark:bg-neutral-700 border-double dark:border-orange-400 h-16 items-center relative shadow w-full z-10 md:h-20 lg:rounded-2xl lg:mx-2"
			>
				<div class="flex flex-center flex-col h-full justify-center mx-auto px-3 relative">
					<div class="flex items-center pl-1 relative w-full sm:pr-2 sm:ml-0 lg:max-w-68">
						...
						>
							<Button
								on:click={toggleMode}
								class="w-12 h-12 flex text-center justify-center align-middle rounded-full"
								variant="outline"
								size="icon"
							>
								<Sun
